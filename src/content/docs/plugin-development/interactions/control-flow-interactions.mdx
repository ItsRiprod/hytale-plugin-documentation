...
```
Frame 1-3: MyPlugin_WindUp (state = NotFinished)
Frame 4:   MyPlugin_WindUp (state = Finished)
           ↓
Frame 5-6: MyPlugin_Swing (state = NotFinished)
Frame 7:   MyPlugin_Swing (state = Finished)
           ↓
Frame 8-9: MyPlugin_Recovery (state = NotFinished)
Frame 10:  MyPlugin_Recovery (state = Finished)
           ↓
Serial complete
```
...
```
                    ┌─── Fork 1: Root_Visual_Effect ──→ (runs independently)
                    │
Main Chain → Parallel ─── Fork 2: Root_Sound_Effect ──→ (runs independently)
                    │
                    └─── Fork 3: Root_Damage_Apply ───→ (runs independently)
                    │
            (state = Finished immediately)
```
...
```
RepeatInteraction (Repeat: 3)
    │
    ├─→ Fork 1: Root_Spin_Attack
    │       ↓ (completes)
    ├─→ Fork 2: Root_Spin_Attack
    │       ↓ (completes)
    └─→ Fork 3: Root_Spin_Attack
            ↓ (completes)
    ↓
Next: MyPlugin_Recovery
```
